...
parser <- OptionParser(
  usage = "usage: %prog folder output_file",
  option_list = list(
    make_option(c("-v", "--verbose"), default=F, help="Verbosity (default False)")
  ),
  add_help_option = T,
  prog = "Export CARNIVAL solution from CarnivalPy to CarnivalR format",
...
save(export_folder(arguments$args[1]), file=arguments$args[2])
