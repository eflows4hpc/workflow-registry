...
from backtrackbb.external.nllgrid import NLLGrid
from backtrackbb.utils.cf.summary_cf import summary_cf
from backtrackbb.utils.data_types.bp_types import Trigger
from backtrackbb.utils.grid.projection import sta_GRD_Proj
from backtrackbb.utils.map.project import get_transform
from backtrackbb.utils.map.project import rect2latlon
from backtrackbb.utils.plotting.plot import bp_plot
...
def run_btbb_task(args):
    return _run_btbb_task(*args)


@task(
    returns=1,
    file_out_fig=FILE_OUT,
...
def run_btbb_no_fig_task(args):
    return _run_btbb_no_fig_task(*args)
...
def __run_btbb__(
    ...
    if config.ignore_noisy_CF:
        sums = [
            (
                "%s" % str(tr.stats.station),
                "%s" % str(tr.stats.channel),
                (tr.data / tr.max()).sum(),
            )
            for tr in st_CF_cut
        ]
        ...
        noisy_sta_wave = [
            (s[0], s[1]) for s in sums if s[2] >= config.n_ignore_noisy_CF * min_sum
        ]
    else:
        noisy_sta_wave = []
    ...
    for sta_wave1, sta_wave2 in itertools.combinations(sta_wave, 2):
        ...
        arglist.append((config, sta_wave1, sta_wave2, sig1, sig2, t_begin, tau_max))
    ...
    for out in outputs:
        ...
        stack_grid.array += proj_function(delta_tt_array.flatten()).reshape(
            delta_tt_array.shape
        )
    ...
    if config.trigger_ellipsoid_max_axis is not None and not do_trigger:
        ...
        if ell.len1 <= max_ax and stack_grid.max() >= config.trigger_ellipsoid:
            do_trigger = True
    ...
    if config.trigger_probability_range is not None and not do_trigger:
        ...
        ni, nj, nk = mask.shape
        idx = slice_indexes(i_max, j_max, k_max, i_range, j_range, k_range, ni, nj, nk)
    ...
    if do_trigger:
        if config.max_subdivide is not None:
            ...
            idx = slice_indexes(i_max, j_max, k_max, sf, sf, sf, ni, nj, nk)
            i1, i2, j1, j2, k1, k2 = idx
        ...
        trigger.center_win = start_tw + config.time_lag / 2.0
        if stack_grid.proj_name != "NONE":
            trigger.lat, trigger.lon = rect2latlon(trigger.x, trigger.y)
        # compute picks from arrivals and theoretical arrivals from grids
        trigger.make_picks(config.stations, config.wave_type, arrival_times, GRD_sta)
        if config.grid_type != config.wave_type:
            # additional theoretical travel time estimation for second phase
            # only done if config.grid_type and config.wave_type have
