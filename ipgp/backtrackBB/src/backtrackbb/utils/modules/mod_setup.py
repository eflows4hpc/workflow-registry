...
from backtrackbb.external.configobj import ConfigObj
from backtrackbb.external.configobj.validate import Validator
from backtrackbb.utils.config.config import Config
...
def _parse_args(progname):
    ...
    if progname == "backtrack2eventdata" or progname == "group_triggers":
        parser.add_argument(
            "trigger_file",
            metavar="trigger_file",
            type=str,
...
def _parse_configspec():
    try:
        configspec_file = os.path.join(
            os.path.dirname(__file__), "..", "..", "cfg", "configspec.conf"
        )
...
def _parse_config(config_file):
    ...
    for hos_sigma_field in ("hos_sigma_P", "hos_sigma_S"):
        ...
        hos_sigma_dict = {key: value for (key, value) in zip(stations, hos_sigma)}
    ...
    # Grid power
    try:
        config_obj["grid_power"] = eval(
            config_obj["grid_power"],
            {"__builtins__": None},
    ...
    if config_obj["grid_power_ellipsoid"] is None:
        ...
        try:
            config_obj["grid_power_ellipsoid"] = eval(
            ...
            sys.stderr.write(
                'Unable to parse "grid_power_ellipsoid". ' 'Using "grid_power".\n'
            )
            config_obj["grid_power_ellipsoid"] = config_obj["grid_power"]
