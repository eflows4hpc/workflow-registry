...
try:
    ...
    from backtrackbb.dummy.parameter import FILE_IN


@task(returns=3, infile=FILE_IN)
def read_locationTremor(infile, config, depth=35.0):
    ...
    for i, hh in enumerate(hours_dic):
        if hh == float(hour):
            xeq, yeq = util_geo_km(
                lon_or, lat_or, epicenter["lon_e"][i], epicenter["lat_e"][i]
            )
            XX.append(xeq)
            YY.append(yeq)
            ZZ.append(depth)
...
@task(returns=3, infile=FILE_IN)
def read_locationEQ(infile, day, hours, lat_zero, lon_zero, depth=35.0):
    day = config.data_day
    ...
    for i, hh in enumerate(hours_dic):
        year = (
            str(epicenter["yyyy"][i])[2:4]
            + "0"
            + str(int(epicenter["mm"][i]))
