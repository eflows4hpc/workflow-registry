...
from backtrackbb.libs.rosenberger import rosenberger
from backtrackbb.utils.recursive.rec_filter import rec_filter_coeff
from backtrackbb.utils.recursive.rec_filter import rec_filter_norm
from backtrackbb.utils.recursive.rec_filter import recursive_filter
from backtrackbb.utils.recursive.rec_rms import recursive_rms
from backtrackbb.utils.recursive.rec_hos import recursive_hos
from backtrackbb.utils.recursive.rec_gauss_filter import recursive_gauss_filter
from backtrackbb.utils.signals.generate import generate_signal_noise2
from backtrackbb.utils.signals.generate import generate_signal_expSin


def make_LinFq(f_min, f_max, delta, nfreq):
    """Calculate linearly spaced frequency array for MB filtering."""
    f_ny = float(1.0 / (2 * delta))
    if f_max > f_ny:
        f_max = f_ny
...
def MBfilter_CF(
    ...
    if len(st) < 2:
        ...
        for n in range(Nb):
            ...
            if CF_type == "kurtosis":
                CF1[n] = recursive_hos(
                    YN1[n], CF_decay_constant, hos_order, hos_sigma, rmem
                )
    ...
    # 2 (horizontal) components analysis
        ...
        for n in range(Nb):
            ...
            YN_E[n] = recursive_filter(y1, CN_HP[n], CN_LP[n], filter_npoles, rmem1)
            YN_E[n] /= filter_norm[n]
            YN_N[n] = recursive_filter(y2, CN_HP[n], CN_LP[n], filter_npoles, rmem2)
            ...
            if CF_type == "kurtosis":
                CF1[n] = recursive_hos(
                    YN1[n], CF_decay_constant, hos_order, hos_sigma, rmem1
                )
    ...
    # 3 components analysis, includes polarization P and S decomposition
        ...
        for n in range(Nb):
            ...
            filt_dataP, filt_dataS, _ = rosenberger(
                YN2[n],
                YN3[n],
                YN1[n],
            ...
            filteredDataS[n] = np.sqrt(
                np.power(filt_dataS[1, :], 2) + np.power(filt_dataS[2, :], 2)
            )
            ...
            if CF_type == "envelope":
                if wave_type == "P":
                    CF1[n] = recursive_rms(filteredDataP[n], CF_decay_constant, rmem1)
                    if full_output:
                        CF2[n] = recursive_rms(
                            filteredDataS[n], CF_decay_constant, rmem2
                        )
                else:
                    CF1[n] = recursive_rms(filteredDataS[n], CF_decay_constant, rmem1)
                    if full_output:
                        CF2[n] = recursive_rms(
                            filteredDataP[n], CF_decay_constant, rmem2
                        )
