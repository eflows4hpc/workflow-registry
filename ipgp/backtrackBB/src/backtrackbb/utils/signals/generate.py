...
def generate_signal_expSin(
    ...
    for i in range(position1, len(noise) - 1):
        signal[i - 1] = signal[i - 1] + function_sinExp(
            i - position1, decreasingFactor1, pulse1
        )
    for i in range(position2, len(noise) - 1):
        signal[i - 1] = signal[i - 1] + amplitude2 * function_sinExp(
            i - position2, decreasingFactor2, pulse2
        )
    return signal
...
def generate_signal_triangle(position, slope, maxValue, lengthSignal, sigmaNoise=0):
    signal = (
        np.random.randn(
            lengthSignal,
        )
