...
class Trigger:
    ...
    def make_picks(self, stations, arrival_types, arrival_times=None, grids=None):
        for sta, arr in product(stations, arrival_types):
            ...
            if grids is not None:
                pick.theor_time = grids[sta][arr].get_value(self.x, self.y, self.z)
            self.add_pick(pick)
    ...
    def compute_origin_time(self, dt_min, update_evid=False):
        ...
        otime = _time_average(
            (
                _pick.pick_time_absolute - _pick.theor_time
        ...
        for pick in self.picks:
            ...
            if pick.pick_time_absolute is not None:
                pick.time_dev = abs(pick.pick_time_absolute - pick.theor_time_absolute)
