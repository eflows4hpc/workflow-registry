...
from backtrackbb.utils.modules.mod_btbb import run_btbb_task
from backtrackbb.utils.modules.mod_btbb import run_btbb_no_fig_task
...
def main_continuous():
    """
    Python code for running BTBB code over continuous data by splitting
    into chunks of 1 hour
    """
    # --- Input parameters
    root_folder = "/gpfs/scratch/bsc19/bsc19234/BackTrackBB_FINAL/backtrackbb-master/scripts/cluster/"
    # -- Path to output folder --
    stream_base = root_folder + "stream/fds"
    outdir_base = root_folder + "out_synthetic_continous_np/2222/01"
    datadir_base = (
        "/gpfs/scratch/bsc44/bsc44755/scratch_pr1ejg00_pr1ejg19/pr1ejg19/data/2222/01"
    )
    # -- ID base file --
    base_config_file = (
        root_folder + "../examples/mn4/Synthetic/BT_SyntheticExample_MN_base.conf"
    )
    #
    year = "2222"
    month = "01"
    ...
    config_path = (
        root_folder + "../examples/mn4/Synthetic/BT_SyntheticExample_running.conf"
    )
    cmd = "cp " + base_config_file + " " + config_path
    os.system(cmd)
