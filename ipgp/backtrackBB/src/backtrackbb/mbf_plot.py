...
from backtrackbb.utils.modules.mod_filter_picker import MBfilter_CF
from backtrackbb.utils.modules.mod_filter_picker import GaussConv
...
def main():
    ...
    st_n = read_traces(config, traces)
    ...
    # ---------------------------MB filtering---------------------------------
    HP2_1, MBkurt_1, Tn2_1, Nb2_1 = MBfilter_CF(
        st,
        config.frequencies,
        config.CN_HP,
        config.CN_LP,
    ...
    lines = LineCollection(
        [list(zip(time_array, trace)) for trace in traces],
    ...
    trans1 = transforms.blended_transform_factory(fig.transFigure, ax2.transData)
    trans2 = transforms.blended_transform_factory(fig.transFigure, ax3.transData)
    ...
    if config.ch_function == "kurtosis":
        ...
        ax4.plot(
            time_array,
            MBkurt_1max_gauss / max(MBkurt_1max_gauss),
            "b",
            label=label,
            lw=1.5,
        )
    ...
    st.filter(
        "bandpass",
        freqmin=config.f_min,
        freqmax=config.f_max,
        corners=2,
        zerophase=False,
    )
    st.normalize()
    if len(st) == 2:
        horiz_env = np.sqrt(np.power(st[0].data, 2) + np.power(st[1].data, 2))
