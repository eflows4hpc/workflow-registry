...
try:
    ...
    from backtrackbb.dummy.constraint import constraint
    from backtrackbb.dummy.task import task
    from backtrackbb.dummy.parameter import INOUT
    from backtrackbb.dummy.parameter import FILE_OUT
    from backtrackbb.dummy.parameter import COLLECTION_IN
    from backtrackbb.dummy.parameter import DICTIONARY_FILE_IN
    from backtrackbb.dummy.parameter import DICTIONARY_IN
    from backtrackbb.dummy.parameter import DIRECTORY_IN
    from backtrackbb.dummy.parameter import Type
    from backtrackbb.dummy.parameter import Depth
    from backtrackbb.dummy.api import compss_wait_on
    from backtrackbb.dummy.api import compss_barrier
    from backtrackbb.dummy.api import compss_delete_object

from backtrackbb.utils.cf.summary_cf import summary_cf
from backtrackbb.utils.cf.summary_cf import empty_cf
from backtrackbb.utils.filter.init_filter import init_filter
from backtrackbb.utils.modules.mod_btbb import init_worker
from backtrackbb.utils.modules.mod_btbb import run_btbb
from backtrackbb.utils.modules.mod_btbb import run_btbb_no_fig
from backtrackbb.utils.modules.mod_setup import configure
from backtrackbb.utils.modules.mod_utils import read_locationEQ
from backtrackbb.utils.modules.mod_utils import read_locationTremor
from backtrackbb.utils.parameters.utils import parse_input_parameters
from backtrackbb.utils.plotting.plot import plt_SummaryOut
from backtrackbb.utils.reading.read_grids import read_grid
from backtrackbb.utils.reading.read_traces import read_traces
from backtrackbb.utils.reading.read_traces_SDS import read_traces_SDS
from backtrackbb.utils.recursive.rec_memory import init_recursive_memory
...
def main(all_config_paths, tw_x_task):
    ...
    for config, st, t_bb in all_configs:
        config = compss_wait_on(
            config
        )  # access later to some config parameters (e.g. recursive_memory)
        st = compss_wait_on(
            st
        )  # access later to inner element (e.g. st[0].stats to get the start time)
...
def main_continuous():
    """
    Python code for running BTBB code over continuous data by
    splitting into chunks of 1 hour
    """
        ...
        for hour in hours:
            ...
            if studycase == "synthetic":
                ...
                starttime = UTCDateTime(
                    "-".join((str(year), str(month), str(day), str(hour)))
                )
                endtime = (
                    UTCDateTime("-".join((str(year), str(month), str(day), str(hour))))
                    + 3600
                    + 10
                )
