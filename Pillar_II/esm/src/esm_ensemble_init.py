...
def esm_ensemble_generate_namelists(exp_id, outpath, start_year, esm_config):
   try:
      # namelist.config
      mapdict = {'{START_YEAR}': start_year , '{MESH_PATH}': esm_config['fesom2']['mesh_file_path'] , '{CLIMATOLOGY_PATH}': esm_config['fesom2']['climatology_path'] , '{OUTPUT_PATH}': esm_config['common']['output_dir'] + start_year + "/" }
